...
def Clear():
    ...
    img1 = PhotoImage(file='images/Photo.png')
...
def Save():
    ...
    if N1 == "" or C1 == "Select Class" or D2 == "" or Re1 == "" or S1 == "" or fathername == "" or mothername == "" or F1 == "" or M1 == "":
        ...
        registration_no() #it will recheck registration no. and reissue no.
...
def search():
    ...
    saveButton.config(state='disable') #after clicking on search , save button will disable so that no one can click it
...
def Update():
    ...
    Clear()  #after update it will clear all entry box
...
Label(root, text="Email: franktabique10@gmail.com", width=10, height=3, bg="#f0687c", anchor='e').pack(side=TOP, fill=X)
...
imageicon4 = PhotoImage(file="images/layer2.png")
Update_button = Button(root, image=imageicon4, bg="#c36464",command=Update)
Update_button.place(x=100, y=64)
...
obj = LabelFrame(root, text="Student's Details", font=('Arial', 20), bd=2, width=900, bg=framebg, fg=framefg, height=250, relief=GROOVE)
...
obj2 = LabelFrame(root, text="Parent's Details", font=('Arial', 20), bd=2, width=900, bg=framebg, fg=framefg, height=220, relief=GROOVE)
...
img1 = PhotoImage(file="images/Photo.png")
lbl = Label(f, bg="black", image=img1)
lbl.place(x=0, y=0)

Button(root, text="Upload", width=19, height=2, font="arial 12 bold", bg="lightblue", command=showimage).place(x=1000, y=380)
